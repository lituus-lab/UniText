...
task coverage, "LCOV + HTML coverage report for the Nim sources (needs lcov)":
  ...
  rmDir capiCache
  exec "nim c --path:src --app:staticlib --noMain --mm:arc -d:release -d:noAutoInit --debugger:native" &
       " --passC:--coverage --passL:--coverage --nimcache:" & capiCache &
       " -o:build/libUniText_cov.a src/UniText/c_api.nim"
  ...
  exec makeExe & " -C tests/c BIN=test_capi_cov" &
       " LIB=../../build/libUniText_cov.a EXTRA_CFLAGS=--coverage"
  ...
  exec "lcov --capture --no-function-coverage --directory " & capiCache &
       " --base-directory ." &
       " --include \"*/src/UniText/*\" --output-file build/capi.info --quiet" &
